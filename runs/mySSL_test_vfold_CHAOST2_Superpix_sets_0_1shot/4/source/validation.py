...
def main(_run, _config, _log):
    if _run.observers:
        ...
        print("check:", f"{_run.observers[0].dir}")
    ...
    _log.info(
        f'###### Reload model from folder/snapshot:  {_config["model_folder"]}/{_config["model_snapshot"]}.pth ######')
    ...
    mar_val_metric_node = Metric(max_label=max_label,
    ...
    with torch.no_grad():
        ...
        for curr_lb in test_labels:
            ...
            support_batched = te_parent.get_support(curr_class=curr_lb, class_idx=[curr_lb],
            ...
            for sample_batched in testloader:
                ...
                if sample_batched["is_start"]:
                    ...
                    outsize = (
                        256, 256, outsize[0])  # original image read by itk: Z, H, W, in prediction we use H, W, Z
                ...
                sup_img_part = [[shot_tensor.unsqueeze(0) for shot_tensor in
                                 support_images[0][q_part]]]  # way(1) x shot x [B(1) x C x H x W]
                ...
                query_pred, _, _, assign_mats = model(sup_img_part, sup_fgm_part, sup_bgm_part, query_images,
                ...
                if (sample_batched["z_id"] - sample_batched["z_max"] <= _config['z_margin']) and (
                    ...
                    mar_val_metric_node.record(query_pred, np.array(query_labels[0].cpu()), labels=[curr_lb],
    ...
    m_classDice, _, m_meanDice, _, m_rawDice = mar_val_metric_node.get_mDice(labels=sorted(test_labels), n_scan=None,
                                                                             give_raw=True)
